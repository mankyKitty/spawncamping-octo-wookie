...
-export([start_link/0, % Starts and links the process in one step
        stop/0,        % stops it
...
-export([init/1,          % initialise our process
         ...
         terminate/2,     % is called on shutdown
...
start_link() ->
    gen_server:start_link( % start_link spawns and links to a new process
                           % in one atomic step The parameters:
...
stop() ->                  % note that we do not use ! anymore. Instead
    gen_server:cast(       % we use case to send a message asynch. to
...
say_hello() ->             % Pretty much the same as stop above except 
    gen_server:cast(       % that we send the atom say_hello instead.
     ...
     say_hello).           % are only interested in a side effect.
...
get_count() ->             % Here, on the other hand, we do expect a 
    gen_server:call(       % response, which is why we use call to 
     ...
     get_count).           % blocks until we get the response. Note how
                           % gen_server:call/2 hides the send/receive logic from us.
%% ------------------------------------------------------------------
...
init([]) ->                      % these are the behaviour callbacks. init/1 is 
    {ok, #state{count=0}}.       % called in response to gen_server:start_link/4
                                 % and we are expected to initialise state.
...
code_change(_OldVsn, State, _Extra) ->      % called during release up/down-grade
    {ok, State}.                            % to update internal state or cycle various things
