...
(defn render-coll [^Graphics g cell]
  (let [[state x y] cell
        x (inc (* x (dim-scale 0)))
        y (inc (* y (dim-scale 1)))]
    (doto g
...
(defn active-neighbours [above [left _ right] below]
  (count
   (filter #(= :on (% 0))
...
(defn rules [above current below]
  (let [[self x y] (second current)]
    (cond
     (= :on self) [:dying x y]
     (= :dying self) [:off x y]
     (= 2 (active-neighbours above current below)) [:on x y]
     :else [:off x y])))
...
(defn activity-loop [surface scope]
  (while true
    (swap! stage step)
    (.repaint surface)))
